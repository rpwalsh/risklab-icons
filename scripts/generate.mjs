...
const bases = [
  ...
  icon('aircraft','transport',p('M3 13l7-2V5l2-2 2 2v6l7 2v2l-7-1v4l2 2v1l-4-1-4 1v-1l2-2v-4l-7 1z'),['flight','aviation']),
  icon('anchor','transport',p('M12 3v14M8 7h8M5 13a7 7 0 0014 0M5 13H2m17 0h3')+c(12,4,1),['marine','port']),
  icon('antenna','communication',p('M12 14v7m-4 0h8M12 14l-4-4m4 4 4-4M6 8a8 8 0 0112 0M3 5a12 12 0 0118 0'),['radio','transmitter']),
  icon('battery','devices',r(3,7,17,10,2)+r(20,10,2,4,.5)+p('M7 12h4m-2-2v4m5-2h3'),['power','energy']),
  icon('beacon','status',p('M8 20h8l-1-8a3 3 0 00-6 0zM4 10l-2-1m18 1l2-1M6 6L4 4m12 2l2-2M12 4V1'),['alert','locator']),
  icon('bookmark','files',p('M6 3h12v18l-6-4-6 4z'),['save','favorite']),
  icon('briefcase','files',r(3,7,18,13,2)+p('M8 7V4h8v3M3 12h18m-11 0v2h4v-2'),['work','case']),
  icon('building','places',p('M4 21V5l8-3 8 3v16M8 7h2m4 0h2M8 11h2m4 0h2M8 15h2m4 0h2M10 21v-3h4v3'),['office','facility']),
  icon('calculator','devices',r(5,2,14,20,2)+r(8,5,8,4,1)+p('M8 13h1m3 0h1m3 0h1M8 17h1m3 0h1m3 0h1'),['math','compute']),
  icon('clipboard','files',r(5,4,14,18,2)+r(8,2,8,5,2)+p('M8 11h8M8 15h8M8 19h5'),['report','task']),
  icon('code','development',p('M9 6l-6 6 6 6m6-12l6 6-6 6m-2-14L11 20'),['developer','source']),
  icon('cube','data',p('M12 2l9 5v10l-9 5-9-5V7zM3 7l9 5 9-5M12 12v10'),['model','volume']),
  icon('dashboard','data',p('M4 4h7v7H4zm9 0h7v4h-7zm0 6h7v10h-7zM4 13h7v7H4z'),['layout','overview']),
  icon('external-link','navigation',p('M14 4h6v6m0-6l-9 9M18 13v7H4V6h7'),['open','launch']),
  icon('fingerprint','security',p('M6 10a6 6 0 0112 0v3M4 10a8 8 0 0116 0v5M8 11v3a4 4 0 004 4m0-10a4 4 0 014 4v4a7 7 0 01-2 5M8 18a7 7 0 01-2-5'),['identity','biometric']),
  icon('flask','science',p('M9 3h6M10 3v6l-5 9a2 2 0 002 3h10a2 2 0 002-3l-5-9V3M7 16h10'),['lab','experiment']),
  icon('help','status',c(12,12,9)+p('M9.5 9a2.7 2.7 0 115 1.5c-.7 1-2.5 1.4-2.5 3M12 17h.01'),['question','support']),
  icon('info','status',c(12,12,9)+p('M12 11v6M12 7h.01'),['information','details']),
  icon('keyboard','devices',r(2,6,20,12,2)+p('M5 10h1m3 0h1m3 0h1m3 0h1M5 14h2m2 0h6m2 0h2'),['input','type']),
  icon('laptop','devices',p('M5 4h14v11H5zM3 19h18l-2-4H5z'),['computer','portable']),
  icon('lightbulb','status',p('M8 15a6 6 0 118 0l-1 2H9zM9 20h6M10 17v-5m4 5v-5'),['idea','insight']),
  icon('map','maps',p('M3 5l6-2 6 2 6-2v16l-6 2-6-2-6 2zM9 3v16m6-14v16'),['terrain','location']),
  icon('microphone','communication',r(9,3,6,11,3)+p('M6 11a6 6 0 0012 0M12 17v4m-4 0h8'),['voice','audio']),
  icon('package','files',p('M4 7l8-4 8 4v10l-8 4-8-4zM4 7l8 4 8-4M12 11v10M8 5l8 4'),['box','delivery']),
  icon('printer','devices',p('M7 8V3h10v5M6 17H3V9h18v8h-3M7 14h10v7H7zM17 11h.01'),['print','document']),
  icon('rocket','transport',p('M9 15l-4 1 3-4m7 3l1 4-4-3M8 12c1-5 4-8 8-9 1 4-1 8-5 11zM7 17l-2 2m4 0l-3 3'),['launch','space']),
  icon('save','actions',p('M4 3h14l2 2v16H4zM8 3v6h8V3M7 21v-8h10v8'),['disk','persist']),
  icon('sliders','actions',p('M4 6h6m4 0h6M4 12h10m4 0h2M4 18h3m4 0h9')+c(12,6,2)+c(16,12,2)+c(9,18,2),['controls','adjust']),
  icon('tag','files',p('M3 11V4h7l11 11-6 6z')+c(7.5,7.5,1),['label','category']),
  icon('terminal','development',r(3,4,18,16,2)+p('M7 9l3 3-3 3m5 0h5'),['shell','console']),
  icon('truck','transport',p('M3 6h11v11H3zm11 4h4l3 4v3h-7z')+c(7,18,2)+c(18,18,2),['vehicle','logistics']),
  icon('warning','status',p('M12 3l10 18H2zM12 9v5m0 3h.01'),['alert','caution']),
  icon('wrench','actions',p('M14 6a4 4 0 005 5l-9 9a2 2 0 01-3-3l9-9a4 4 0 01-5-5l2 3z'),['tool','repair']),
];
...
const svg = (body) => `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false">${body}</svg>\n`;
for (const definition of icons) await writeFile(join(iconDir, `${definition.name}.svg`), svg(definition.body), 'utf8');
